...
SURVEYS = {
    "1": "What is your age group? Reply with A (18-25), B (26-35), "
         "C (36-45), D (46+)",
    "2": "How often do you exercise? A (Daily), B (2-3 times a "
         "week), C (Once a week), D (Rarely)",
    "3": "How would you rate your overall health? A (Excellent), "
...
def register_number(phone_number):
    ...
    if response.status_code == 200:
        welcome_msg = (
            "Welcome to our health survey! To start, reply "
            "with:\n'1' for Age survey\n'2' for Exercise"
...
def get_messages():
    """
    Retrieve all messages for the team from the API and update
    user data.

    Returns: list: A list of message dictionaries if successful,
...
def process_message(phone_number, message):
    """
    Process an incoming message, either starting a survey or
    ...
    Returns: str: The next question, a completion message, or an
    error message.
    """
    ...
    if message in SURVEYS:
        ...
        for survey_num, question in SURVEYS.items():
            if question not in user_data[phone_number]["responses"]:
                user_data[phone_number]["responses"][
                    question] = message.upper()
                return (
                    f"Thank you for your response. To take another "
                    f"survey, reply with '1', '2', or '3'.\nOr view"
                    f"your results at: "
                    f"http://yourwebsite.com/results/{phone_number}")
        return ("You've completed all surveys. To retake a survey, "
                "reply with '1', '2', or '3'.")
    else:
        return (
            "Invalid response. Please reply with '1', '2', or '3' "
            "to start a survey, or A, B, C, or D to answer a"
            "question.")
...
def get_results(phone_number):
    ...
    Returns: dict: A dictionary containing the questions and
    responses for all surveys taken.
    """
